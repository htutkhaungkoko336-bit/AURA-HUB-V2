...
function showWaitingRoom() {
    // ၁။ မှတ်ပုံတင်ထားတဲ့ page ကို ဖျောက်မယ်
    const proofPage = document.getElementById('page-payment-proof');
    if (proofPage) {
        proofPage.style.display = 'none';
    }

    // ၂။ Waiting Room ကို ပေါ်လာအောင် လုပ်မယ်
    const matchCenter = document.getElementById('page-match-center');
    if (matchCenter) {
        matchCenter.style.display = 'flex'; // သို့မဟုတ် 'block' (သင်သုံးထားတဲ့ layout ပေါ်မူတည်ပါတယ်)
    }
}