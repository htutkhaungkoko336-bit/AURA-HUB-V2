import { sendMessage } from './bot.js'; // .js ထည့်ရန်မမေ့ပါနဲ့
...
export async function notify(type, data, registrationId) {
    ...
    // Inline Keyboard ထည့်သွင်းခြင်း
    const keyboard = {
        inline_keyboard: [
            [
    ...
    // Main Group သို့ Keyboard နဲ့ ပို့ခြင်း
    const targetChatId = groupIds[type];
    if (targetChatId) {
        // sendMessage function က keyboard ကို လက်ခံနိုင်အောင် ပြင်ထားဖို့လိုမယ်
        await sendMessage(targetChatId, message, keyboard);
    }

    // Admin Alert (Keyboard မလိုပါ)
